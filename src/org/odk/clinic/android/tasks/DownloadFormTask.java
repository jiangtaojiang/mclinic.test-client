...
public class DownloadFormTask extends DownloadTask {
				...
				for (int i = 1; i < values.length; i++) {
					try {
						...
						if (!insertSingleForm(path)) {
							return "Database Exception: " + "Are the ODK Collect databases initialized?" ;
						}
	...
	private boolean insertSingleForm(String formPath) {
		...
		if (!addMe.getName().startsWith(".") && (addMe.getName().endsWith(".xml") 
				|| addMe.getName().endsWith(".xhtml"))) {
			...
			try {
				...
				Log.e("DownloadFormTask", e.getLocalizedMessage() != null 
			...
			Cursor mCursor;
			try {
				mCursor = App.getApp().getContentResolver()
