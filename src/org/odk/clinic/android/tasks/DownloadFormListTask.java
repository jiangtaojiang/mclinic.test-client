...
public class DownloadFormListTask extends DownloadTask {
    ...
    private void insertForms(Document doc) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
    		...
    		Cursor c = mPatientDbAdapter.fetchForm(Integer.valueOf(formId));
    		if (c == null || c.getCount() < 1) { //new form
    			Form f = new Form();
    			f.setName(formName);
    			f.setFormId(Integer.valueOf(formId));
    			mPatientDbAdapter.createForm(f);
    		} else
    			mPatientDbAdapter.updateFormName(Integer.valueOf(formId), formName);
    		c.close();
